...
function waitForProcessExit(pid, timeoutMs = 1e3) {
...
function processHasEnvironmentValue(pid, key, expectedValue, platform = process.platform, execFn = execSync2) {
  const expectedEntry = `${key}=${expectedValue}`;
  try {
    if (platform === "linux") {
      const environment = fs4.readFileSync(`/proc/${pid}/environ`, "utf-8");
...
function getProcessStartTime(pid, platform = process.platform, execFn = execSync2) {
  try {
    const command = platform === "win32" ? `powershell -NoProfile -Command "(Get-Process -Id ${pid}).StartTime.ToUniversalTime().Ticks"` : `ps -o lstart= -p ${pid}`;
...
// src/server/start.ts
var SERVER_OWNERSHIP_ENV = "PROOFSHOT_SERVER_TOKEN";
var DevServerStartError = class extends Error {
  constructor(message, recoveryState) {
    super(message);
...
async function ensureDevServer(command, port, startupTimeout, logPath, onSpawn) {
  if (await isPortOpen(port)) {
    throw new Error(
      `Port ${port} is already in use. Omit --run to use the existing server, or choose another port.`
    );
  }
...
var SESSION_LOCK_FILENAME = ".session.lock";
function saveSession(state) {
  const sessionPath = path4.join(state.outputDir, SESSION_FILENAME);
  fs7.writeFileSync(sessionPath, JSON.stringify(state, null, 2) + "\n");
}
function reserveOutputSession(outputDir, sessionName) {
  const lockPath = path4.join(outputDir, SESSION_LOCK_FILENAME);
  fs7.writeFileSync(
    lockPath,
    JSON.stringify({
      pid: process.pid,
...
function hasActiveSession(outputDir) {
  return fs7.existsSync(path4.join(outputDir, SESSION_FILENAME)) || fs7.existsSync(path4.join(outputDir, SESSION_LOCK_FILENAME));
}
function clearSession(outputDir, sessionName) {
  const sessionPath = path4.join(outputDir, SESSION_FILENAME);
  const lockPath = path4.join(outputDir, SESSION_LOCK_FILENAME);
  if (fs7.existsSync(sessionPath) && (sessionName === void 0 || readSessionName(sessionPath) === sessionName)) {
    fs7.unlinkSync(sessionPath);
  }
  if (fs7.existsSync(lockPath) && (sessionName === void 0 || readSessionName(lockPath) === sessionName)) {
    fs7.unlinkSync(lockPath);
  }
}
function readSessionName(filePath) {
  try {
...
var PROOFSHOT_STATE_DIRECTORY = "proofshot";
var SESSION_REGISTRY_DIRECTORY = "sessions";
var SessionSelectionError = class extends Error {
...
function getSessionRegistryDir(env = process.env, homeDir = os2.homedir()) {
  const stateHome = env.XDG_STATE_HOME || path5.join(homeDir, ".local", "state");
  return path5.join(stateHome, PROOFSHOT_STATE_DIRECTORY, SESSION_REGISTRY_DIRECTORY);
}
function registerSession(session, registryDir = getSessionRegistryDir()) {
  validateSessionName(session.sessionName);
  fs8.mkdirSync(registryDir, { recursive: true });
  const registryPath = getRegistryPath(session.sessionName, registryDir);
  const temporaryPath = `${registryPath}.${process.pid}.tmp`;
  fs8.writeFileSync(temporaryPath, JSON.stringify(session, null, 2) + "\n");
  fs8.renameSync(temporaryPath, registryPath);
}
function reserveSession(session, registryDir = getSessionRegistryDir()) {
  validateSessionName(session.sessionName);
  fs8.mkdirSync(registryDir, { recursive: true });
  fs8.writeFileSync(
...
function unregisterSession(sessionName, registryDir = getSessionRegistryDir()) {
  ...
  if (fs8.existsSync(registryPath)) {
    fs8.unlinkSync(registryPath);
  }
}
function listRegisteredSessions(registryDir = getSessionRegistryDir()) {
  if (!fs8.existsSync(registryDir)) {
    return [];
  }
  return fs8.readdirSync(registryDir).filter((fileName) => fileName.endsWith(".json")).map((fileName) => readRegisteredSession(path5.join(registryDir, fileName))).filter((session) => session !== null).sort((left, right) => right.startedAt.localeCompare(left.startedAt));
}
function resolveSession(options = {}) {
  ...
  const selectableSessions = options.activeBrowserSessionNames instanceof Set ? sessions.filter(
    (session) => options.activeBrowserSessionNames?.has(session.sessionName)
  ) : sessions;
  const workingDirectory = canonicalizeDirectory(
    options.workingDirectory || process.cwd()
  );
  const directoryMatches = selectableSessions.filter(
    (session) => session.startDirectory !== void 0 && canonicalizeDirectory(session.startDirectory) === workingDirectory
  );
...
function listActiveBrowserSessionNames() {
...
function formatSessionChoices(sessions) {
  if (sessions.length === 0) {
...
function getRegistryPath(sessionName, registryDir) {
  return path5.join(registryDir, `${sessionName}.json`);
}
...
function readRegisteredSession(registryPath) {
  try {
    const parsed = JSON.parse(fs8.readFileSync(registryPath, "utf-8"));
    return isSessionState(parsed) ? parsed : null;
  } catch {
    return null;
  }
}
function loadLegacySession(outputDir) {
  return outputDir ? loadSession(path5.resolve(outputDir)) : null;
}
function canonicalizeDirectory(directory) {
  let canonicalPath;
  try {
    canonicalPath = fs8.realpathSync.native(directory);
  } catch {
    canonicalPath = path5.resolve(directory);
  }
...
function isSessionState(value) {
  ...
  const session = value;
  return typeof session.startedAt === "string" && isOptionalString(session.startDirectory) && isOptionalLifecycleStatus(session.lifecycleStatus) && isOptionalNullableNumber(session.ownerPid) && (typeof session.description === "string" || session.description === null) && typeof session.outputDir === "string" && typeof session.sessionDir === "string" && typeof session.sessionName === "string" && isOptionalNullableString(session.browserConfigPath) && isOptionalBoolean(session.headless) && typeof session.videoPath === "string" && typeof session.serverErrorLog === "string" && typeof session.port === "number" && (typeof session.serverCommand === "string" || session.serverCommand === null) && isOptionalNullableString(session.serverOwnershipToken) && isOptionalNullableString(session.serverProcessStartTime) && isOptionalNullableNumber(session.serverPid) && typeof session.serverAlreadyRunning === "boolean" && typeof session.recordingActive === "boolean" && isOptionalViewport(session.viewport);
}
...
function isOptionalLifecycleStatus(value) {
  return value === void 0 || value === "starting" || value === "active" || value === "stopped";
}
function isOptionalViewport(value) {
  if (value === void 0) {
    return true;
  }
  if (typeof value !== "object" || value === null) {
...
function isSessionStillStarting(session) {
  return session.lifecycleStatus === "starting" && session.ownerPid !== null && session.ownerPid !== void 0 && isProcessRunning(session.ownerPid);
}
function stopOwnedServer(session) {
  if (session.serverAlreadyRunning || session.serverPid === null || session.serverPid === void 0) {
    return "not-owned";
  }
  if (!isProcessRunning(session.serverPid)) {
    return "already-stopped";
  }
  const hasOwnershipToken = session.serverOwnershipToken !== null && session.serverOwnershipToken !== void 0 && processHasEnvironmentValue(
    session.serverPid,
    SERVER_OWNERSHIP_ENV,
    session.serverOwnershipToken
  ) === true;
  const hasMatchingStartTime = process.platform === "win32" && session.serverProcessStartTime !== null && session.serverProcessStartTime !== void 0 && getProcessStartTime(session.serverPid) === session.serverProcessStartTime;
...
function discardSession(session, options = {}) {
  setAgentBrowserDefaults({
    configPath: session.browserConfigPath ?? void 0
  });
...
function writeMetadata(sessionDir, metadata) {
  const metadataPath = path6.join(sessionDir, METADATA_FILENAME);
  fs9.writeFileSync(metadataPath, JSON.stringify(metadata, null, 2) + "\n");
}
function loadMetadata(sessionDir) {
  const metadataPath = path6.join(sessionDir, METADATA_FILENAME);
  if (!fs9.existsSync(metadataPath)) return null;
  try {
    return JSON.parse(fs9.readFileSync(metadataPath, "utf-8"));
  } catch {
    return null;
  }
}
function findSessionsForBranch(outputDir, branch) {
  if (!fs9.existsSync(outputDir)) return [];
  const entries = fs9.readdirSync(outputDir, { withFileTypes: true });
  const matches = [];
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const sessionDir = path6.join(outputDir, entry.name);
...
async function startCommand(options) {
  ...
  const outputDir = path7.resolve(config.output);
  const timestamp = generateTimestamp();
  const registeredSession = listRegisteredSessions().find(
    (session2) => path7.resolve(session2.outputDir) === outputDir
  );
    ...
    if (options.force) {
      if (existingSession) {
        if (isSessionStillStarting(existingSession)) {
          console.log(
            chalk2.yellow("\u26A0 A session is still starting.") + chalk2.dim(" Wait for it to finish or stop its ProofShot process first.")
          );
          return;
        }
        try {
          ...
          console.error(chalk2.red("\u2717") + ` Could not clear stale session: ${message}`);
          process.exit(1);
        }
      } else {
  ...
  const sessionSuffix = randomUUID2().slice(0, 8);
  const sessionDirName = `${generateSessionDirName(timestamp, options.description || null)}-${sessionSuffix}`;
  const sessionDir = path7.join(outputDir, sessionDirName);
  const sessionName = generateAgentBrowserSessionName(`${timestamp}-${sessionSuffix}`);
  ensureOutputDir(sessionDir);
  const videoPath = path7.join(sessionDir, "session.webm");
  const serverErrorLog = path7.join(sessionDir, "server.log");
  let branch = "";
  let commitSha = "";
  try {
  ...
  const session = {
    ...
    serverPid: null,
    serverAlreadyRunning: options.run === void 0,
  ...
  if (options.run) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.error(chalk2.red("\u2717") + ` Failed to start dev server: ${message}`);
      process.exit(1);
    }
  } else {
...
` + chalk2.dim("Make sure agent-browser is installed: npm install -g agent-browser") + (cleanupComplete ? "" : chalk2.dim("\nServer cleanup failed; recovery state was preserved."))
    );
    process.exit(1);
  }
...
` + chalk2.dim("Recording is required \u2014 ProofShot cannot proceed without video capture.\n") + chalk2.dim("Troubleshooting:\n") + chalk2.dim("  1. Make sure agent-browser is installed and running\n") + chalk2.dim('  2. Try "proofshot clean" then re-run "proofshot start"\n') + chalk2.dim("  3. If the port was already in use, stop the old server first") + (cleanupComplete ? "" : chalk2.dim("\nServer cleanup failed; recovery state was preserved."))
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.error(
      chalk2.red("\u2717") + ` Failed to persist session state: ${message}` + (cleanupComplete ? "" : chalk2.dim("\nCleanup failed; recovery state was preserved."))
    );
    process.exit(1);
  }
  console.log("");
  ...
  function terminateStartedServer(pid) {
    if (pid === null) {
      return true;
    }
    try {
  ...
  function cleanupFailedSession(failedSession) {
  ...
  function persistRecoverySession(recoverySession) {
  ...
  function applyServerState(targetSession, server) {
...
function writeViewer(outputDir, data) {
  ...
  if (!entries) {
    const logPath = path8.join(outputDir, "session-log.json");
    if (!fs10.existsSync(logPath)) return null;
    try {
      entries = JSON.parse(fs10.readFileSync(logPath, "utf-8"));
    } catch {
      return null;
    }
  }
  ...
  const viewerPath = path8.join(outputDir, "viewer.html");
  fs10.writeFileSync(viewerPath, html);
  return viewerPath;
}
...
function loadSessionLog(sessionDir) {
  const logPath = path9.join(sessionDir, SESSION_LOG_FILENAME);
  if (!fs11.existsSync(logPath)) return [];
  try {
    return JSON.parse(fs11.readFileSync(logPath, "utf-8"));
  } catch {
    return [];
  }
}
function resolveScreenshotPath(args, sessionDir) {
  ...
  if (path9.isAbsolute(screenshotPath)) return args;
  const resolved = path9.join(sessionDir, screenshotPath);
...
async function execCommand(args, options = {}) {
  const action = args.join(" ");
  const config = loadConfig();
  const outputDir = path9.resolve(config.output);
  let activeBrowserSessionNames = null;
  try {
...
${formatSessionChoices(error.sessions)}`
  ...
  const browserConfigPath = session?.browserConfigPath === void 0 ? config.browser.configPath : session.browserConfigPath ?? void 0;
  setAgentBrowserDefaults({
  ...
  if (session) {
    ...
    const logPath = path9.join(session.sessionDir, SESSION_LOG_FILENAME);
    const entries = loadSessionLog(session.sessionDir);
    entries.push(entry);
    fs11.writeFileSync(logPath, JSON.stringify(entries, null, 2) + "\n");
  }
...
function tryClaudeCodeLogs(startTimeMs, endTimeMs) {
  const claudeDir = path10.join(os3.homedir(), ".claude", "sessions");
  if (!fs12.existsSync(claudeDir)) return null;
  try {
    const files = fs12.readdirSync(claudeDir).filter((f) => f.endsWith(".json"));
    for (const file of files) {
      const data = JSON.parse(fs12.readFileSync(path10.join(claudeDir, file), "utf-8"));
...
function estimateFromContent(sessionDir) {
  ...
  try {
    const entries = JSON.parse(fs12.readFileSync(logPath, "utf-8"));
...
async function stopCommand(options) {
  const config = loadConfig();
  const legacyOutputDir = path11.resolve(config.output);
  let activeBrowserSessionNames = null;
  try {
    activeBrowserSessionNames = listActiveBrowserSessionNames();
  } catch {
  ...
  try {
    ...
    if (error instanceof SessionSelectionError) {
      console.error(
        chalk3.red("\u2717") + ` ${error.message}
` + chalk3.dim(`Use --session <id> to choose one:
${formatSessionChoices(error.sessions)}`)
      );
      process.exit(1);
    }
  ...
  if (isSessionStillStarting(session)) {
    console.error(
      chalk3.red("\u2717") + " This ProofShot session is still starting.\n" + chalk3.dim("Wait for start to finish before stopping it.")
    );
    process.exit(1);
  }
  const browserConfigPath = session.browserConfigPath === void 0 ? config.browser.configPath : session.browserConfigPath ?? void 0;
  ...
  if (consoleOutput.trim()) {
    fs13.writeFileSync(path11.join(session.sessionDir, "console-output.log"), consoleOutput);
  }
  ...
  let serverLog = "";
  let serverEntries = [];
  if (fs13.existsSync(session.serverErrorLog)) {
    const rawServerLog = fs13.readFileSync(session.serverErrorLog, "utf-8");
  ...
  const screenshots = fs13.existsSync(sessionDir) ? fs13.readdirSync(sessionDir).filter((f) => f.endsWith(".png")) : [];
  const sessionLog = loadSessionLog(sessionDir);
  let trimOffsetSec = 0;
  if (fs13.existsSync(session.videoPath)) {
  ...
  const summaryPath = path11.join(sessionDir, "SUMMARY.md");
  ...
  fs13.writeFileSync(summaryPath, summary);
  ...
  if (trimOffsetSec > 0 && viewerEntries.length > 0) {
    const logPath = path11.join(sessionDir, "session-log.json");
    fs13.writeFileSync(logPath, JSON.stringify(viewerEntries, null, 2) + "\n");
  }
  ...
  const viewerPath = writeViewer(sessionDir, {
    ...
    serverCommand: session.serverCommand,
    durationSec,
    videoFilename: fs13.existsSync(session.videoPath) ? path11.basename(session.videoPath) : null,
    consoleErrorCount,
    serverErrorCount,
    consoleOutput,
    serverLog,
  ...
  if (serverCleanupError !== null) {
    console.error("");
    console.error(
      chalk3.yellow("\u26A0") + ` ${serverCleanupError.message}
` + chalk3.dim(
...
function generateProofSummary(data) {
  const date = (/* @__PURE__ */ new Date()).toISOString().replace("T", " ").slice(0, 19);
  const projectName = path11.basename(data.projectDirectory);
...
`;
  ...
  const relativeVideo = path11.basename(data.videoPath);
  md += `## Video Recording
...
function trimVideo(videoPath, screenshots, outputDir, recordingStartMs, sessionLog) {
  ...
  if (sessionLog.length > 0) {
    ...
    const timestamps = screenshots.map((f) => {
      try {
        return fs13.statSync(path11.join(outputDir, f)).birthtimeMs;
      } catch {
        return null;
      }
  ...
  const ext = path11.extname(videoPath);
  const base = path11.basename(videoPath, ext);
  const rawPath = path11.join(dir, `${base}-raw${ext}`);
  try {
    fs13.renameSync(videoPath, rawPath);
    execSync6(
    ...
    if (fs13.existsSync(videoPath)) {
      fs13.unlinkSync(videoPath);
    }
    if (fs13.existsSync(rawPath)) {
      fs13.renameSync(rawPath, videoPath);
    }
...
function validateTrimmedVideo(videoPath) {
  if (!fs13.existsSync(videoPath) || fs13.statSync(videoPath).size === 0) {
...
async function diffCommand(options) {
  const config = loadConfig();
  const currentDir = path12.resolve(config.output);
  const baselineDir = path12.resolve(options.baseline);
  if (!fs14.existsSync(baselineDir)) {
    console.error(chalk4.red("\u2717") + ` Baseline directory not found: ${baselineDir}`);
    process.exit(1);
  }
  if (!fs14.existsSync(currentDir)) {
    console.error(
...
` + chalk4.dim('Run "proofshot verify" first to generate screenshots.')
  ...
  const baselineFiles = fs14.readdirSync(baselineDir).filter((f) => f.startsWith("page-") && f.endsWith(".png"));
  const currentFiles = fs14.readdirSync(currentDir).filter((f) => f.startsWith("page-") && f.endsWith(".png"));
  if (baselineFiles.length === 0) {
    console.error(chalk4.red("\u2717") + " No baseline screenshots found (looking for page-*.png)");
    process.exit(1);
  }
  const diffDir = path12.join(currentDir, "diffs");
  fs14.mkdirSync(diffDir, { recursive: true });
  console.log(chalk4.dim("Comparing screenshots...\n"));
  let hasChanges = false;
  for (const file of baselineFiles) {
    const baselinePath = path12.join(baselineDir, file);
    const currentPath = path12.join(currentDir, file);
    const diffPath = path12.join(diffDir, `diff-${file}`);
    if (!fs14.existsSync(currentPath)) {
...
async function uploadAsset(filePath, token, repoId) {
  ...
  const fileSize = fs16.statSync(filePath).size;
...
GitHub response: ${body}`
  ...
  const fileBuffer = fs16.readFileSync(filePath);
...
async function uploadAssetsToRepoContents(options) {
  ...
  for (let i = 0; i < options.filePaths.length; i += 1) {
    ...
    try {
      const content = fs16.readFileSync(filePath, "base64");
      const uploadPath = path14.posix.join(
        options.uploadRoot,
        path14.basename(path14.dirname(filePath)),
        fileName
      );
      await githubApi(
...
` + chalk6.dim('Run "proofshot start" and "proofshot stop" first.')
  ...
  for (const sessionDir of sessionDirs) {
    ...
    for (const f of files) {
      if (f.endsWith(".png")) {
        screenshotPaths.push(path15.join(sessionDir, f));
      }
    }
    if (!videoPath) {
      for (const f of files) {
        if (f === "session.webm" || f === "session.mp4") {
          videoPath = path15.join(sessionDir, f);
          break;
        }
      }
    }
    const summaryPath = path15.join(sessionDir, "SUMMARY.md");
    if (fs17.existsSync(summaryPath)) {
      const summary = fs17.readFileSync(summaryPath, "utf-8");
  ...
  if (options.dryRun) {
    ...
    const commentData2 = {
      ...
      video: videoPath ? {
        url: `https://github.com/user-attachments/assets/<${path15.basename(videoPath)}>`,
        renderMode: "embed"
      } : null,
      errorCount,
      branch,
...
function screenshotLabel(ssPath) {
  const sessionDir = path15.basename(path15.dirname(ssPath));
  const fileName = path15.basename(ssPath);
...
function buildUploadRoot(branch, prNumber, commitSha) {
  ...
  return path15.posix.join("proofshot", `pr-${prNumber}`, sanitizedBranch, `${timestamp}-${sha}`);
}
...
async function doctorCommand() {
  ...
  const legacySession = loadSession(path16.resolve(outputDir));
  const sessions = registeredSessions.length > 0 ? registeredSessions : legacySession ? [legacySession] : [];
  ...
  console.log("");
  printLine("Sessions", sessions.length === 0 ? chalk7.dim("none") : String(sessions.length));
...
async function sessionListCommand(options) {
  ...
  let activeBrowserSessions = null;
  try {
    activeBrowserSessions = listActiveBrowserSessionNames();
  } catch {
  }
  const entries = sessions.map(
    (session) => buildSessionListEntry(session, activeBrowserSessions)
  );
  if (options.json) {
  ...
  console.log("");
  for (const entry of entries) {
    const status = entry.status === "active" ? chalk8.green(entry.status) : entry.status === "starting" ? chalk8.cyan(entry.status) : entry.status === "orphaned" ? chalk8.yellow(entry.status) : chalk8.dim(entry.status);
    ...
    console.log(chalk8.dim(`  Port:    ${entry.port}`));
  }
}
async function sessionCleanCommand(options) {
  ...
  if (options.all) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.error(chalk8.red(`Unable to inspect agent-browser sessions: ${message}`));
      process.exit(1);
    }
  }
  const sessionsToClean = options.all ? sessions.filter((session) => !isSessionStillStarting(session)) : sessions.filter(
    (session) => !isSessionStillStarting(session) && !activeBrowserSessions.has(session.sessionName)
  );
  ...
  for (const session of sessionsToClean) {
    try {
      ...
      failures += 1;
      const message = error instanceof Error ? error.message : String(error);
      console.error(`${chalk8.red("\u2717")} Kept ${session.sessionName}: ${message}`);
    }
  }
  if (failures > 0) {
    process.exitCode = 1;
...
function buildSessionListEntry(session, activeBrowserSessions) {
  const status = isSessionStillStarting(session) ? "starting" : activeBrowserSessions === null ? "unknown" : activeBrowserSessions.has(session.sessionName) ? "active" : "orphaned";
  return {
...
function createCLI() {
  ...
  program2.command("stop").description("Stop session: stop recording, collect errors, bundle proof artifacts").option("--no-close", "Don't close the browser (keep it open for further use)").option("--session <id>", "Target a specific active ProofShot session").action(async (options) => {
  ...
  program2.command("exec").description("Run an agent-browser command with logging (use instead of agent-browser directly)").option("--session <id>", "Target a specific active ProofShot session").argument("<args...>", "agent-browser command and arguments").passThroughOptions().allowUnknownOption().action(async (args, options) => {
    await execCommand(args, options);
  });
  const session = program2.command("session").description("List and clean registered ProofShot sessions");
  ...
  session.command("clean").description("Remove orphaned ProofShot sessions").option("--all", "Stop and remove all registered ProofShot sessions").action(async (options) => {
