...
function createCLI() {
  ...
  program.command("stop").description("Stop session: stop recording, collect errors, bundle proof artifacts").option("--no-close", "Don't close the browser (keep it open for further use)").option("--session <id>", "Target a specific active ProofShot session").action(async (options) => {
  ...
  program.command("exec").description("Run an agent-browser command with logging (use instead of agent-browser directly)").option("--session <id>", "Target a specific active ProofShot session").argument("<args...>", "agent-browser command and arguments").passThroughOptions().allowUnknownOption().action(async (args, options) => {
    await execCommand(args, options);
  });
  const session = program.command("session").description("List and clean registered ProofShot sessions");
