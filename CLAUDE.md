...
- `stop` detects a mid-session capture gap two ways, because the workers fail in two shapes: a surviving pid file with a dead process means the helper was killed, while a tmux pane that exited closes its pipe as a clean EOF and is only visible through `#{pane_pipe}` — so the pane check must run before teardown detaches the pipes. Either way `stop` records the gap and exits non-zero after finishing teardown
