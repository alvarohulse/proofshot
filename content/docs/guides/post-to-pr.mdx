...
ProofShot auto-selects only when exactly one complete `PASS` or `FAIL` session matches the PR head. It never combines sessions.
...
When more than one compatible session exists, choose one and optionally narrow its screenshots:

```sh
proofshot pr 42 --session proofshot-2026-08-09_19-00-00
proofshot pr 42 --session proofshot-2026-08-09_19-00-00 --screenshot checkout.png receipt.png
```
...
2. One compatible finalized manifest is selected
