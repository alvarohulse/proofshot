...
| `--session <id>` | Select one finalized session explicitly |
| `--screenshot <artifact...>` | Publish only the selected screenshot IDs, paths, or unique basenames |
...
2. Selects exactly one complete `PASS` or `FAIL` manifest compatible with that head, or requires `--session`
