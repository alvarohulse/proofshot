...
| `network-summary.json` | `stop` | Sanitized metadata-only network request summary. |
| `step-*.png` | `exec screenshot` | Screenshots captured at key moments. Filenames come from the `exec screenshot` argument. |
| `USER_TESTING.md` | `replay` | High-level human testing instructions from the replay case. |
...
Raw dev-server output is stored in `private/server.log`; raw browser console evidence is under `private/browser/`; and HAR/request bodies are under `private/agent-browser/`. The `private/` directory is excluded from manifests and publication.
